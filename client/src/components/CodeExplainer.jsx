...
function CodeExplainer({ addToHistory, viewingHistoryItem }) {
  ...
  return (
    <div className="w-full max-w-6xl bg-black p-4 sm:p-6 rounded-lg shadow-lg">
      <h2 className="text-xl sm:text-2xl mb-4 text-center text-white">Code Explainer</h2>
      <h3 className="text-lg sm:text-xl mb-2 text-white">Paste Your Code Snippet:</h3>
      <textarea
        className="w-full h-48 sm:h-64 p-3 bg-[#222222] rounded-md border border-[#444444] font-mono text-sm sm:text-base text-white resize-none"
        value={code}
      ...
      {explanation && (
        <div className="mt-6 p-4 bg-[#111111] rounded-md border border-[#333333]">
          <h3 className="text-lg sm:text-xl mb-2 text-white">Explanation:</h3>
          <pre className="text-[#bbbbbb] whitespace-pre-wrap font-sans text-sm sm:text-base leading-relaxed overflow-x-auto">{explanation}</pre>
        </div>
      )}
    </div>
  );
