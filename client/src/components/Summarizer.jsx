...
function Summarizer({ addToHistory, viewingHistoryItem }) {
  ...
  return (
    <div className="w-full max-w-4xl bg-black p-4 sm:p-6 rounded-lg shadow-lg">
      <h2 className="text-xl sm:text-2xl mb-4 text-center text-white">Text Summarizer</h2>
      <textarea
        className="w-full h-32 sm:h-48 p-3 bg-[#222222] rounded-md border border-[#444444] text-sm sm:text-base text-white resize-none"
        value={text}
      ...
      {summary && (
        <div className="mt-6 p-4 bg-[#111111] rounded-md border border-[#333333]">
          <h3 className="text-lg sm:text-xl mb-2 text-white">Summary:</h3>
          <p className="text-[#bbbbbb] whitespace-pre-wrap text-sm sm:text-base leading-relaxed">{summary}</p>
        </div>
      )}
    </div>
  );
